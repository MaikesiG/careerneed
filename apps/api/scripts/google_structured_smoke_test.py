"""Manual local Gemini smoke test only. Never invoke from CI."""

import os
from collections.abc import Callable
from time import monotonic

OPT_IN_ENV_VAR = "CAREERNEED_RUN_GOOGLE_SMOKE_TEST"
OPT_IN_INSTRUCTION = "Set CAREERNEED_RUN_GOOGLE_SMOKE_TEST=1 to run this manual local smoke test."
UNAVAILABLE_MESSAGE = "No configured Google model is available for the smoke test."
UNEXPECTED_FAILURE_MESSAGE = "Google structured-output smoke test failed unexpectedly."
...
def _run_smoke_test(router_factory: Callable[[], object] | None) -> int:
    ...
    try:
        router = factory()
        google_models = tuple(
            model for model in router.available_models() if model.provider == "google"
        )
        if not google_models:
            print(UNAVAILABLE_MESSAGE)
            return 1

        selected_model = google_models[0]
        ...
        if isinstance(error, GeminiUnhandledProviderError):
            print(f"failure_stage={error.failure_stage}")
            print(f"exception_module={error.exception_module}")
            print(f"exception_type={error.exception_name}")
            if error.http_status is not None:
                print(f"http_status={error.http_status}")
            diagnostic = error.unicode_diagnostic
            if diagnostic is not None:
                print("Unicode encoding diagnostic:")
                print(f"stage={diagnostic.stage}")
                print(f"encoding={diagnostic.encoding}")
                print(f"start={diagnostic.start}")
                print(f"end={diagnostic.end}")
                print(f"reason={diagnostic.reason}")
                print("Gemini field diagnostic:")
                for field in diagnostic.fields:
                    print(
                        f"{field.name}: present={str(field.present).lower()} "
                        f"ascii={str(field.ascii_compatible).lower()} "
                        f"length={field.length} "
                        f"leading_whitespace={str(field.leading_whitespace).lower()} "
                        f"trailing_whitespace={str(field.trailing_whitespace).lower()}"
                    )
        return 1

    print("Google structured-output smoke test succeeded.")
