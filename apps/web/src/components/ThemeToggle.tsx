...
export default function ThemeToggle() {
  ...
  /** * During SSR and the initial hydration: * * Server -> Monitor * Client -> Monitor * * This prevents hydration mismatch when `theme` or * `resolvedTheme` is different between server and client. * * After hydration: * * light -> Sun * dark -> Moon * system -> Monitor */ const TriggerIcon =
    !mounted ? Monitor : activeTheme === "system" ? Monitor : resolvedTheme === "dark" ? Moon : Sun;
          ...
          {options.map(({ value, label, Icon }) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm transition-colors ${isSelected ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
              >
                {" "}
