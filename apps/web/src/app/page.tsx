    ...
    <main className="min-h-screen bg-slate-950 px-6 py-16 text-slate-100">
      <section className="mx-auto max-w-4xl">
        ...
        <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
          A job-discovery and career-intelligence platform for technical professionals. Today:
          establish a reliable development foundation.
        </p>
        <div className="mt-10 rounded-2xl border border-slate-700 bg-slate-900 p-6">
          ...
          {error && (
            <p className="mt-2 text-xl font-semibold text-rose-400">API unavailable: {error}</p>
          )}
          {!health && !error && (
            <p className="mt-2 text-xl font-semibold text-amber-300">Checking API…</p>
          )}
        </div>
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div key={item} className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
