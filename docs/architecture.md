...
CareerNeed is a job-search workspace. It allows a user to keep discovered opportunities, manually found roles, resume data, application progress, notes, and follow-up work in a single system.
...
- Rendering Home, Resumes, Jobs, Applications, Sources, and manual job-entry pages.
...
- Resume, job, application, and source resource access.
...
### 4.4 Company/source configuration
...
→ Application record, notes, follow-up
```
...
→ Applications page groups and surfaces follow-up work
```
...
- Treat resumes, application notes, and follow-up information as sensitive user data.
